...
export default function DeshVideshForm() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('JS login error:', error); // Debug
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (uploadError) {
        setMessage('फोटो अपलोड में एरर: ' + uploadError.message);
        console.log('Supabase image upload error:', uploadError); // Debug
        throw uploadError;
      }
      ...
      console.log('JS image upload error:', error); // Debug
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        console.log('Supabase delete error:', error); // Debug
        throw error;
      }
      ...
      console.log('JS delete error:', error); // Debug
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editId) {
        ...
        if (error) {
          setMessage('Error: ' + error.message);
          console.log('Supabase update error:', error); // Debug
          throw error;
        }
        ...
        if (error) {
          setMessage('Error: ' + error.message);
          console.log('Supabase insert error:', error); // Debug
          throw error;
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        ...
        {/* देश-विदेश List */}
        <div className="my-8">
          <h3 className="text-lg font-bold mb-2">सारी देश-विदेश खबरें</h3>
          <ul>
            {deshVideshList.map(item => (
              <li key={item.id} className="mb-6 border-b pb-4">
                ...
                {item.image_url && (
                  <img src={item.image_url} alt="Image" className="max-h-32 mt-2 rounded" />
                )}
